...
public class ParserService {
  ...
  public ResponseEntity<ResponseDto<LintingResultDto>> lint(
    ...
    try {
      ...
      return FullResponse.create(
          ...
          "lintResult",
          new LintingResultDto(snippetId, report.getFullReport().getReports(), language, version, rules),
          HttpStatus.OK);
    } catch (Exception e) {
      return FullResponse.create(
          "Linting failed", "lintResult", new LintingResultDto(snippetId, List.of(), language, version, rules), HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  public ResponseEntity<ResponseDto<ValidateResultDto>> validate(
      Long snippetId, ProcessingRequestDto request) {
    String language = request.getLanguage().toLowerCase();
    String version = request.getVersion();
    ...
    if (snippet.isEmpty()) {
      return FullResponse.create(
          "Snippet not found", "validationResult", null, HttpStatus.NOT_FOUND);
    }
    ...
    return FullResponse.create(
        "Validation executed",
        "validationResult",
        new ValidateResultDto(report, state, language, version),
        HttpStatus.OK);
  }
}
